...
@inject(Keys.labelOrder, Keys.detailOrder)
@observer
export default class Print extends Component {

  componentWillUnmount() {
    this.props.labelOrder.clear();
  }

  render() {
    const {dataSource, fetching} = this.props.labelOrder;
    const columns = [{
      title: 'Mã đơn hàng',
      dataIndex: 'Code',
      key: 'Code',
    ...
    const rowSelection = {
      selectedRowKeys: this.props.labelOrder.code,
      onChange: (selectedRowKeys, selectedRows) => {
        console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
        this.props.labelOrder.changeRowKeys(selectedRowKeys);
      }
    };
          ...
          <div style={{marginTop: 16}}>
            <Spin spinning={fetching}>
              <Table
                ...
                pagination={this.props.labelOrder.pagination}
                onChange={this.props.labelOrder.handleTableChange}
              />
            </Spin>
          </div>
        </ContentHolder>
