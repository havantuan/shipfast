...
@inject(Keys.uploadOrderByStaff)
@observer
export default class OrderTable extends React.PureComponent {
  ...
  render() {
    let {response, selectedRowKeys} = this.uploadOrderByStaff;
    const columns = [{
      title: 'Đơn hàng',
      key: 'order',
      ...
      render: (text, record, index) =>
        <div>
          ...
          <p>
            ...
            <span className="font-bold">Phát: </span> {ObjectPath.get(record, 'FullAddress')}
            {
    ...
    const rowSelection = {
      selectedRowKeys,
      onChange: this.uploadOrderByStaff.onSelectChange,
    };
